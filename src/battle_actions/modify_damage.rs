...
pub fn modify_damage(
    ...
    // }
        ...
        debug_elog!("[MODIFY_DAMAGE] Parental Bond modifier: {}", bond_modifier);
        base_damage = battle.modify_f(base_damage, bond_modifier);
    }

    // baseDamage = this.battle.runEvent("WeatherModifyDamage", pokemon, target, move, baseDamage);
    debug_elog!("[MODIFY_DAMAGE] turn={}, Before WeatherModifyDamage event: base_damage={}", battle.turn, base_damage);
    ...
    match weather_result {
        EventResult::Number(modified) => {
            ...
            debug_elog!("[MODIFY_DAMAGE] turn={}, After WeatherModifyDamage event (Number): base_damage={}", battle.turn, base_damage);
        }
        EventResult::Float(multiplier) => {
            ...
            debug_elog!("[MODIFY_DAMAGE] turn={}, After WeatherModifyDamage event (Float {}x): base_damage={}", battle.turn, multiplier, base_damage);
        }
        _ => {
            debug_elog!("[MODIFY_DAMAGE] turn={}, WeatherModifyDamage returned: {:?}", battle.turn, weather_result);
        }
    }
    ...
    debug_elog!("[MODIFY_DAMAGE] turn={}, BEFORE randomizer: base_damage={}, should_apply={}",
        battle.turn, base_damage, should_apply_variance);

    if should_apply_variance {
        base_damage = battle.randomizer(base_damage);
    }
    debug_elog!("[MODIFY_DAMAGE] turn={}, AFTER randomizer (applied={}): base_damage={}",
    ...
    let (source_types, _target_types, target_slot, source_terastallized, source_pre_tera_types, source_stellar_boosted_types, source_species_name) = {
        ...
        let target_types = if let Some(pokemon) = battle.pokemon_at(target_pos.0, target_pos.1) {
            if battle.turn >= 64 && battle.turn <= 66 {
                debug_elog!("[MODIFY_DAMAGE] Reading target types for {} (species: {}): raw={:?}",
                    pokemon.name, pokemon.species_id, pokemon.types);
            }
    ...
    if battle.turn >= 64 && battle.turn <= 66 {
        debug_elog!("[MODIFY_DAMAGE] move_type={}, source_types={:?}", move_type, source_types);
    }

    debug_elog!("[MODIFY_DAMAGE] turn={}, About to check STAB: move_type={}, source_types={:?}",
    ...
    if &move_type != "???" {
        ...
        if battle.turn >= 64 && battle.turn <= 66 {
            debug_elog!("[MODIFY_DAMAGE] Checking STAB: has_stab={}", has_stab);
        }
        debug_elog!("[MODIFY_DAMAGE] turn={}, STAB check: has_stab={}, base_damage_before={}",
        ...
        debug_elog!("[MODIFY_DAMAGE] turn={}, After STAB (stab={}): base_damage={}", battle.turn, stab, base_damage);
    } else {
        debug_elog!("[MODIFY_DAMAGE] turn={}, Skipping STAB because move_type is ???", battle.turn);
    }
    ...
    debug_elog!("[MODIFY_DAMAGE] Type effectiveness (via run_effectiveness): move_id={}, type_mod={}",
    ...
    if type_mod > 0 {
        ...
        debug_elog!("[MODIFY_DAMAGE] After super effective: base_damage={}", base_damage);
    } else if type_mod < 0 {
        battle.add("-resisted", &[Arg::String(target_slot.clone())]);
        for _ in type_mod..0 {
            base_damage = battle.trunc(base_damage as f64 / 2.0, None) as i32;
        }
        debug_elog!("[MODIFY_DAMAGE] After resisted: base_damage={}", base_damage);
    }
        ...
        && !source_has_guts
    {
        if battle.gen < 6 || active_move.id.as_str() != "facade" {
            ...
            debug_elog!("[MODIFY_DAMAGE] After burn halving: base_damage={}", base_damage);
        }
    }
    ...
    debug_elog!("[MODIFY_DAMAGE] Before ModifyDamage event: base_damage={}", base_damage);
    if let EventResult::Number(modified) = battle.run_event(
        ...
        base_damage = modified;
        debug_elog!("[MODIFY_DAMAGE] After ModifyDamage event: base_damage={}", base_damage);
    }
    ...
    debug_elog!("[MODIFY_DAMAGE] FINAL: result={}", result);
    result
}
